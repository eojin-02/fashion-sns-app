...
| 필드 | 방식 | 근거 |
|---|---|---|
| category (세부 15~20클래스) | **파인튜닝 분류 (2계층)** | 핵심 목표. 모델은 세부 카테고리를 예측하고 5슬롯은 매핑 표로 도출 |
...
### 1.1 2계층 분류 체계 (세부 → 5슬롯)

모델 출력은 세부 카테고리(한국어 라벨, DB `category`에 그대로 저장),
아바타용 5슬롯은 매핑 표로 자동 도출한다. avatar_builder.slot_of()가 이미
세부 키워드("후드", "청바지", "부츠" 등)를 퍼지 매칭하므로 아바타 파이프라인 무수정.

| 슬롯 | 세부 클래스 (초안 — prepare_data가 표본 수 기준으로 확정) |
|---|---|
| 상의 | 티셔츠, 셔츠·블라우스, 니트, 후드·맨투맨 |
| 하의 | 청바지, 슬랙스, 반바지, 스커트, 레깅스 |
| 아우터 | 자켓·블레이저, 코트, 패딩, 집업·가디건 |
| 신발 | 스니커즈, 구두·힐, 부츠, 샌들·슬리퍼 |
| 액세서리 | 모자, 가방, 기타 |
| (특수) | 원피스 — v1 슬롯 매핑은 상의, 3단계(형태 다양화)에서 통짜 몸통으로 |

세분화 상한 규칙: **클래스당 학습 표본 500장 미만이면 상위 클래스로 자동 병합**
(prepare_data.py가 클래스 히스토그램을 출력하고 병합 결과를 로그로 남긴다).
평가도 이중으로: 세부 macro-F1 ≥ 0.85 **그리고** 5슬롯으로 접었을 때 ≥ 0.92
(세부에서 틀려도 슬롯이 맞으면 아바타·코디 기능은 무사하다).
