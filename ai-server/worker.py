...
"box"는 그 아이템이 사진에서 차지하는 영역 — 반드시 0~1 사이 소수 비율 좌표
(왼쪽 위 원점, 예: [0.31, 0.42, 0.68, 0.77]). 픽셀 좌표 금지.
...
def analyze_with_gemini(image_bytes: bytes) -> dict:
    ...
    # 주 모델이 혼잡(503)하면 lite로 폴백 — 스캔이 일시 장애에 통째로 죽지 않게
    last_error = None
    for model in (settings.GEMINI_MODEL, "gemini-flash-lite-latest"):
        try:
            ...
            log.warning("Gemini %s 호출 실패(%s) — 폴백 시도", model, type(e).__name__)
            last_error = e
    else:
        raise last_error
...
def rebuild_avatar(user_id: int) -> None:
    ...
    with psycopg.connect(settings.DATABASE_URL) as conn:
        ...
        # 사진 크롭 텍스처 로드 — 무늬/주름 음영을 파트에 래핑 (실패 시 단색 폴백)
        for entry in outfit.values():
            crop_key = entry.get("crop_key")
            if not crop_key:
                continue
            try:
                from PIL import Image
                body = s3.get_object(Bucket=settings.S3_BUCKET, Key=crop_key)["Body"].read()
                entry["texture"] = Image.open(io.BytesIO(body)).convert("RGB")
            except Exception:
                log.warning("크롭 텍스처 로드 실패 key=%s — 단색 폴백", crop_key)
